...
export const addListeningActivity = async (req, res, next) => {
	try {
		...
		const listeningEntry = new ListeningHistory({
...
export const getRecentActivity = async (req, res, next) => {
	try {
		...
		const recentActivity = await ListeningHistory.find({ userId: currentUserId })
			.sort({ playedAt: -1 })
			.limit(parseInt(limit))
			.select('songTitle artistName imageUrl playedAt duration completed');
